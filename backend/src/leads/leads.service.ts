...
export class LeadsService {
  ...
  async findDuplicate(dto: Pick<CreateLeadDto, 'phone' | 'email'>) {
    ...
    const lead = await this.prisma.lead.findFirst({ where: { OR: conditions } });
  ...
  async list(user: AuthUser) {
    ...
    return this.prisma.lead.findMany({
      where: unrestricted ? {} : { OR: [{ assignedToId: user.sub }, { createdById: user.sub }] },
  ...
  async get(id: string, user: AuthUser) {
    ...
    const lead = await this.prisma.lead.findFirst({
      where: { id, ...(unrestricted ? {} : { OR: [{ assignedToId: user.sub }, { createdById: user.sub }] }) },
