...
type Lead = { id: string; organization: string; contactName?: string; phone?: string; whatsapp?: string; email?: string; status: string; nextFollowupAt?: string; duplicateOfId?: string; duplicateReason?: string };
...
export default function LeadsPage() {
  ...
  async function load() {
    try { setLeads(await api<Lead[]>('/leads')); } catch (e: any) { setError(e.message); }
  }
  useEffect(() => { load(); try { setRole(JSON.parse(localStorage.getItem('rk_crm_user') || '{}').role || ''); } catch {} }, []);
  ...
  return (
    <AppShell>
      <PageHeader title="My Leads" subtitle="Create manual leads, view assigned clients, and catch duplicate contacts before they enter the system." />
      <div className="grid grid-cols-3 gap-6">
        <form onSubmit={createLead} className="card p-6">
          ...
          <div className="mt-5 space-y-3">
            ...
            {['OWNER','MANAGER'].includes(role) && <label className="flex items-center gap-2 rounded-xl bg-amber-50 p-3 text-sm font-semibold text-amber-800"><input type="checkbox" checked={allowDuplicateOverride} onChange={(e) => setAllowDuplicateOverride(e.target.checked)} /> Override exact duplicate phone/email if found</label>}
        ...
        <section className="card col-span-2 p-6">
          ...
          <table className="w-full text-left text-sm">
            <thead className="text-xs uppercase text-slate-400"><tr><th className="py-3">Organization</th><th>Contact</th><th>Phone</th><th>Status</th><th>Next Follow-up</th><th>Actions</th></tr></thead>
            <tbody>
              {leads.map((lead) => (
                <tr key={lead.id} className="border-t border-slate-100">
                  <td className="py-4 font-semibold">{lead.organization}{lead.duplicateReason && <div className="mt-1 text-xs font-bold text-amber-600">Duplicate contact found</div>}</td>
                  <td>{lead.contactName || '-'}</td><td>{lead.phone || '-'}</td><td><StatusBadge status={lead.status} /></td>
                  <td>{lead.nextFollowupAt ? new Date(lead.nextFollowupAt).toLocaleString() : '-'}</td>
                  <td className="space-x-2"><a href={lead.phone ? `tel:${lead.phone}` : '#'}>☎</a><a href={lead.whatsapp ? `https://wa.me/${lead.whatsapp}` : '#'}>🟢</a><Link href={`/leads/${lead.id}`}>View</Link></td>
                </tr>
              ))}
