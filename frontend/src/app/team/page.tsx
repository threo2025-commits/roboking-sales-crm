  ...
  return (
    <AppShell>
      ...
      <div className="grid grid-cols-3 gap-6">
        <form onSubmit={createUser} className="card p-6">
        ...
        <section className="card col-span-2 p-6">
          ...
          {resetRequests.map((r) => (
            <div key={r.id} className="mb-3 flex items-center justify-between rounded-xl border p-4 text-sm">
              <div><b>{r.user.name}</b> ({r.user.loginId}) requested reset <div className="text-slate-500">{r.reason || 'No reason'}</div></div>
              <button onClick={() => resetPassword(r.user.id, r.id)} className="rounded-xl bg-slate-950 px-4 py-2 font-bold text-white">Reset Password</button>
            </div>
          ))}
      ...
      <section className="card mt-6 p-6">
        ...
        <form onSubmit={connectEmail} className="mt-4 grid grid-cols-3 gap-3">
